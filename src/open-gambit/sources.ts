...
export interface DiscoveryResult {
  ...
  /**
   * Items dropped by the version-noise pre-filter before snapshots, candidates
   * and signals. Persisted in `gambit_discovery_stats.version_noise_items` so a
   * future zero-pass run can tell "sources published nothing" apart from "the
   * new filter removed everything".
   */
 ...
 */
/**
 * Version-noise pre-filter (Phase 1, T6.4).
 *
 * A weekly release feed publishes one entry per version, and most of those
 * entries are a version number plus an autogenerated changelog. They are dropped
 * HERE, before snapshots, candidates and signals, so routine version churn can
 * never compete for a per-source item slot with a real announcement, and can
 * never reach the LLM.
 *
 * Two shapes are filtered:
 *
 *   1. Pre-release markers anywhere in the title (`nightly`, `canary`, `-rc1`,
 *      `.rc0`, `-alpha`, `-beta`, `-dev`, version build `-stage`). These are not
 *      publication candidates at all, with or without a body.
 *   2. A title that is ONLY a version or package version (`v1.5.0`,
 *      `wrangler@4.131.0`, `@cloudflare/pages-shared@0.13.179`, `Release v0.59.0`)
 *      whose body is too short (`VERSION_ONLY_BODY_CHARS`) to carry an event
 *      plus its object and a mechanism.
 *
 * An over-long body is deliberately NOT filtered: a release whose body actually
 * describes a strategic event must stay eligible, and the deterministic signals
 * decide it. `preview` alone is not a pre-release marker here either -- it is
 * ordinary prose ("in public preview") and appears in real announcements.
 */
const PRERELEASE_TITLE = /(?:^|[\s@\-_.])(?:nightly|canary)(?:[\s.\-]|$)|[\-._](?:rc|alpha|beta|dev|stage|check-stage)\d*(?:[\s.\-]|$)/iu;
const VERSION_ONLY_TITLE = /^(?:release[:\s]+)?(?:@?[\w./-]+@)?\[?\s*v?\d+(?:\.\d+)*(?:[-.][\w.-]+)?\s*\]?$/iu;
/** Below this body length a version-only title cannot carry a strategic event. */
const VERSION_ONLY_BODY_CHARS = 200;

export function isVersionNoiseItem(title: string, content: string): boolean {
  const normalizedTitle = title.trim();
  if (!normalizedTitle) return true;
  if (PRERELEASE_TITLE.test(normalizedTitle)) return true;
  return VERSION_ONLY_TITLE.test(normalizedTitle) && content.trim().length < VERSION_ONLY_BODY_CHARS;
}
...
export async function discoverConfiguredSources(
  ...
  for (const entry of fetched) {
    ...
    for (const feedItem of result.feedItems) {
      ...
      if (isVersionNoiseItem(feedItem.title, feedItem.content)) {
        versionNoiseItems += 1;
        continue;
      }
